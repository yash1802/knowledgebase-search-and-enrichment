...
->->-> manual_input.py       # Handling information manually entered via chat
...
->->-> sqlite_store.py       # Relational database (SQLite) 
...
1. Chunking Logic - Located in src/ingestion/document_processor.py, the system employs content-aware chunking strategies based on file type.
...
2. Retrieval Logic - The RetrievalEngine in src/rag/retrieval.py implements a sophisticated two-stage retrieval process.
...
3. LLM Logic - The LLMClient (src/llm/llm_client.py) manages interactions with OpenAI.

- Model: Uses gpt-4o-mini for a balance of speed, cost and intelligence. 

- Intent Routing: A dedicated classification step determines whether the user is asking a question, providing new information or is simply making conversation. 

- Determinism: Sets temperature=0.0 to ensure consistent, reproducible outputs.



4. Prompt Engineering - Prompts are centralized in src/llm/prompts.py.
